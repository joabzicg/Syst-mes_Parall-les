...
class Grille:
    ...
    def compute_next_iteration(self):
        ...
        """
        # Remarque 1: ici, on utilise une mise à jour vectorisée (convolution) plutôt que des boucles Python.
        # Remarque 2: comme dans la version à boucles, l'affichage considère (0,0) en bas à gauche.
        ny = self.dimensions[0]
        nx = self.dimensions[1]
        # Convolution 3x3 (centre=0) pour compter les 8 voisines.
        kernel = np.ones((3, 3), dtype=np.uint8)
        kernel[1, 1] = 0
        voisins = convolve2d(self.cells, kernel, mode='same', boundary='wrap').astype(np.uint8)

        # Règles de Conway (discrètes) :
        # - naissance si exactement 3 voisines
        # - survie si vivante et 2 ou 3 voisines
        alive = self.cells == 1
        born = (~alive) & (voisins == 3)
        survive = alive & ((voisins == 2) | (voisins == 3))
        next_cells = np.zeros(self.dimensions, dtype=np.uint8)
        next_cells[born | survive] = 1

        diff_mask = next_cells != self.cells
        diff_cells = (np.flatnonzero(diff_mask)).tolist()
